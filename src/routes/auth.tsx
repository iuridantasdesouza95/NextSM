...
function AuthPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithPassword({ email: String(form.get("login-email") ?? ""), password: String(form.get("login-pass") ?? "") });
    setLoading(false); if (error) return toast.error(error.message); toast.success("Bem-vindo à NextSM!");
    if (nextPath) { window.location.href = nextPath; return; } navigate({ to: "/areas", replace: true });
  }

  async function handleSignup(e: React.FormEvent) {
    e.preventDefault(); setLoading(true); const form = new FormData(e.currentTarget as HTMLFormElement);
    const nome = String(form.get("reg-nome") ?? ""); const depto = String(form.get("reg-depto") ?? "");
    const { error } = await supabase.auth.signUp({ email: String(form.get("reg-email") ?? ""), password: String(form.get("reg-pass") ?? ""), options: { emailRedirectTo: window.location.origin + "/areas", data: { nome, departamento: depto } } });
    setLoading(false); if (error) return toast.error(error.message); toast.success("Conta criada! Verifique seu e-mail se necessário.");
  }

  function handleNextIdLogin() { window.location.assign(`/oauth/login${nextPath ? `?next=${encodeURIComponent(nextPath)}` : ""}`); }
  ...
  return (
    <div className="nextsm-auth min-h-screen overflow-x-hidden overflow-y-auto">
      <div className="nextsm-auth__orb nextsm-auth__orb--one" /><div className="nextsm-auth__orb nextsm-auth__orb--two" /><div className="nextsm-auth__grid" /><div className="nextsm-network-bg" aria-hidden="true" />
      <div className="relative mx-auto grid min-h-screen max-w-7xl items-start gap-8 px-4 py-6 sm:px-6 sm:py-10 lg:items-center lg:gap-12 lg:grid-cols-[1fr_460px] lg:px-12">
        <section className="hidden lg:block"><NextSMLogo inverse className="mb-10" /><div className="max-w-xl"><p className="mb-4 text-sm font-semibold uppercase tracking-[0.22em] text-cyan-300">Service Desk</p><h1 className="text-5xl font-semibold leading-[1.05] tracking-tight text-white xl:text-6xl">Estrutura inteligente para uma operação que não para.</h1><p className="mt-6 max-w-lg text-base leading-7 text-slate-300">Centralize atendimento, processos, conhecimento e automação em uma experiência de Service Management moderna.</p></div></section>
