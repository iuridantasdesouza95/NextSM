-- Corrige a persistência e consulta do feedback de utilidade da Base de Conhecimento.
-- A avaliação é única por usuário/artigo e fica protegida por auth.uid().
...
CREATE OR REPLACE FUNCTION public.obter_feedback_artigo_base_conhecimento(p_artigo_id uuid)
RETURNS jsonb
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
